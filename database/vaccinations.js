...
export const infoTable = async (date) => {
  return new Promise((resolve, reject) => {
    Connection.query(
      ...
      WITH table1 AS (SELECT o.healthCareDistrict, o.id, o.injections, o.arrived, COUNT(v.id) AS numOfVaccsGiven, o.injections - COUNT(v.id) AS unUsedInjections
          FROM vaccinations.order o
      ...
      SELECT healthCareDistrict, SUM(unUsedInjections) as expiringInjections
      FROM table1
      GROUP BY healthCareDistrict
      ),
      vaccinesExpiringInTenDays as (
        WITH table10 AS (
          SELECT o.healthCareDistrict, o.id, o.injections, o.arrived, COUNT(v.id) AS numOfVaccsGiven, o.injections - COUNT(v.id) AS unUsedInjections, DATEDIFF("2021-04-12", DATE(o.arrived))
          FROM vaccinations.order o
          LEFT JOIN  vaccinations.givenvaccinations v ON o.id = v.sourceBottle
          WHERE DATEDIFF("2021-04-12", o.arrived) BETWEEN 20 and 29
          GROUP BY o.healthCareDistrict, o.id, o.injections, o.arrived
          HAVING NumOfVaccsGiven < o.injections
          ORDER BY o.healthCareDistrict)
        
        SELECT healthCareDistrict, SUM(unUsedInjections) as expiringVaccinesTenDays
        FROM table10
        GROUP by healthCareDistrict
        )
      
      SELECT dar.healthCareDistrict, dar.date, dar.arrivedInjections, dus.usedInjections, ved.expiringInjections, ve10.expiringVaccinesTenDays 
